...
import {
  ...
  type SlippageTier,
} from "@/hooks/use-slippage-timeseries";
...
function ChartTooltip({
  ...
  const grouped = new Map<
    string,
    { ask?: number; bid?: number; color: string }
  >();
  for (const entry of payload) {
    ...
    if (!grouped.has(exchange)) {
      grouped.set(exchange, {
        color: EXCHANGE_COLORS[exchange as ExchangeKey] ?? "#888",
      });
    }
    ...
    >
      ...
      {Array.from(grouped.entries()).map(([exchange, g]) => (
        <div key={exchange} style={{ marginBottom: 4 }}>
          ...
          <div
            style={{ display: "flex", gap: 12, fontSize: 11, color: "#9fb0d1" }}
          >
...
export function SlippageHistoryChart({
  ticker,
  activeExchanges,
  timeframe,
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-center gap-1">
        ...
        {isLoading && (
          <span className="ml-2 text-xs text-[var(--text-muted)] animate-pulse">
            Loading…
          </span>
        )}
      </div>
